...
namespace CMGDBEditor.MVVM
{
    public class NoteSequenceCommands(NoteSequencesView vm)
    {
        ...
        public void AddNoteSequence()
        {
            ...
            vm.EditorPanel = new NoteSequenceEditorPanel(vm);
            vm.ModifyMode = "Add";
        ...
        public async void EditNoteSequence(string name)
        {
            ...
            if (response == null)
            {
                vm.Status = new Message() { Text="Note Sequence not found", Error = true };
                return;
            }
        ...
        public async void SubmitNoteSequence()
        {
            ...
            if (vm.ModifyMode == "Add")
            {
                ...
                if (!response)
                {
                    vm.Status = new Message() { Text = "Unknown error occurred while adding note sequence.", Error = true };
            ...
            else
            {
                ...
                if (!response)
                {
                    vm.Status = new Message() { Text = "Unknown error occurred while modifying note sequence.", Error = true };
                    vm.EditorPanel = new BlankPanel();
                    return;
                }
            }
            vm.Status = new Message()
            {
        ...
        public async void DeleteNoteSequence(string name)
        {
            ...
            if (result == MessageBoxResult.Yes)
            {
                ...
                if (!response)
                {
                    vm.Status = new Message()
                    {
                        Text = $"Note sequence '{name}' not found.",
                        Error = true
                    };
                    return;
                }
        ...
        public async void ListNoteSequences()
        {
            ...
            if (response == null)
            {
                vm.Status = new Message() { Text = "Failed to load note sequences.", Error = true };
                return;
            }
            vm.NoteSequenceList = response;
            vm.NotifyPropertyChanged(nameof(vm.NoteSequenceList));
            vm.Status = new Message() { Text = $"{vm.NoteSequenceList.Count} note sequences loaded.", Error = false };
        }
    }
}
