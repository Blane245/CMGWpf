...
namespace CMGWpf.Layout
{
    ...
    // This control provide a visual representation of all of the track generators that appear in the current timeline view. It is responsible for rendering the generators in the track display canvas, and for handling user interactions with the generators. Each generator is displayed as a rectangle starting at its start time and ending at this end time, with the height being 1/3 of the track height. The rectangle contains the name of the generator. When the rectangle is right-clicked, it provides a menu of several functions. The background color of the rectangle depends on the genrator's type. The generator functions include: 
    ...
    // In addition to these functions, the generator rectangle can be left-clicked/dragged to moved the generator rectangle up and down within the track display canvas to avoid obscuration with other generators.
